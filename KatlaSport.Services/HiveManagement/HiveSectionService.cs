...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        {
            var dbHiveSections = await _context.Sections
                .Where(s => s.Code == updateRequest.Code && s.Id != hiveSectionId)
            ...
            dbHiveSections = await _context.Sections.Where(s => s.Id == hiveSectionId).ToArrayAsync().ConfigureAwait(false);
            if (dbHiveSections.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
        ...
        {
            var dbHiveSections = await _context.Sections.Where(s => hiveSectionId == s.Id).ToArrayAsync().ConfigureAwait(false);
